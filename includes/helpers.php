...
class Helpers {
  ...
  public function set_terms_count( $terms ) {
    foreach ( $terms as $index => $term ) {
      ...
      $term_query_results = new \WP_Query( [
				'post_type'      => self::$post_type,
				'posts_per_page' => -1,
				'post_status'    => self::$post_type === 'attachment' ? ['inherit', 'private'] : ['any', 'trash'],
				'fields'         => 'ids',
				'tax_query'      => [
					[
  ...
  public static function get_all_items( $post_type = 'attachment', $wp_query_args = [] ) {
		$attachment_args = array_merge( $wp_query_args, [
			'post_type'      => $post_type,
			'post_status'    => $post_type === 'attachment' ? ['inherit', 'private'] : ['any', 'trash'],
			'posts_per_page' => -1,
      'fields'         => 'ids',
		] );
  ...
  public static function can_edit_media_categories() {
	...
	public static function get_open_category() {
		// List view get HTML via AJAX
		if ( isset( $_GET[self::$taxonomy_name] ) ) {
			return intval( $_GET[self::$taxonomy_name] );
		}
